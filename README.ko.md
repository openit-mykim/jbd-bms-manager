...
현재 알파(alpha.10)에서는 밸런스·보호·온도·용량 설정과 캘리브레이션(전류·셀 전압·NTC), MOS 제어(충·방전 FET 차단/허용), 백업 및 복원(내보내기·일괄 복원)의 `조회 → 수정/측정 → 확인 → 적용 → 재읽기 검증` 흐름을 제공합니다. 밸런스 델타는 5mV 단위로 조절할 수 있고, 셀별 안전 밴드(정상/주의/위험)와 V·mV 병기 표시를 제공합니다. 실기기 물리 검증은 마스터 확인 대기 중입니다.
