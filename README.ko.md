...
- 제어 탭 유지보수: 설정(밸런스/보호/온도/용량)·캘리브레이션·MOS 제어·백업/복원 조회·수정·적용·재검증(알파.10)
- 밸런스 진단: 5mV 단위 밸런스 델타 조절, 셀 안전 밴드 색상(정상/주의/위험), 셀별 V+mV 표시(알파.10)
- 시각화: 개요 SOC 게이지(3단계 밴드), 상세 이력 추세 차트(24시간/7일 · SOC/팩 전압/셀 편차), 기기 로컬 10초 샘플·7일 보관(알파.11)
...
현재 알파(alpha.11)에서는 밸런스·보호·온도·용량 설정과 캘리브레이션(전류·셀 전압·NTC), MOS 제어(충·방전 FET 차단/허용), 백업 및 복원(내보내기·일괄 복원)의 `조회 → 수정/측정 → 확인 → 적용 → 재읽기 검증` 흐름을 제공합니다. 밸런스 델타는 5mV 단위로 조절할 수 있고, 셀별 안전 밴드(정상/주의/위험)와 V·mV 병기 표시를 제공합니다. 실기기 물리 검증은 마스터 확인 대기 중입니다.
