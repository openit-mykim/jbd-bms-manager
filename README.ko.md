...
- 제어 탭 유지보수: 설정(밸런스/보호/온도/용량)·캘리브레이션·MOS 제어·백업/복원 조회·수정·적용·재검증(알파.9)
...
현재 알파(alpha.9)에서는 밸런스·보호·온도·용량 설정과 캘리브레이션(전류·셀 전압·NTC), MOS 제어(충·방전 FET 차단/허용), 백업 및 복원(내보내기·일괄 복원)의 `조회 → 수정/측정 → 확인 → 적용 → 재읽기 검증` 흐름을 제공합니다. 실기기 물리 검증은 마스터 확인 대기 중입니다.
